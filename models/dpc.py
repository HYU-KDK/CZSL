...
def dpc_init(train_dataset, config, device, prompt_template="a photo of X X", csp_checkpoint_path=None, num_prototypes=1):
    ...
    if csp_checkpoint_path and os.path.exists(csp_checkpoint_path):
        ...
        # Multi-prototype support: [N_prim * K, D]
        soft_embedding_prim = torch.zeros(
            ((len(attributes) + len(classes)) * num_prototypes, orig_token_embedding.size(-1)),
            device=device
        )
        for idx, rep in enumerate(orig_token_embedding):
            eos_idx = tokenized[idx].argmax()
            token_mean = torch.mean(rep[1:eos_idx, :], axis=0)
            
            # Fill all K prototypes with the same initial mean (with small noise for diversity)
            for k in range(num_prototypes):
                noise = torch.randn_like(token_mean) * 0.01 if num_prototypes > 1 else 0
                soft_embedding_prim[idx * num_prototypes + k, :] = token_mean + noise
