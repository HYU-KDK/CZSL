        ...
        if config.experiment_name in ['dpc', 'dpc_alpha', 'dpas']:
            ...
            with torch.no_grad():
                ...
                if config.experiment_name == 'dpc_alpha' and 'alpha_predictor' in checkpoint:
                    ...
                    if 'advanced_alpha_predictor' in checkpoint:
                         # Check for architectural mismatch
                         ckpt_in_features = checkpoint['advanced_alpha_predictor']['0.weight'].shape[1]
                         curr_in_features = model.advanced_alpha_predictor[0].in_features
                         if ckpt_in_features != curr_in_features:
                             print(f"Adapting advanced_alpha_predictor architecture: {curr_in_features} -> {ckpt_in_features} in_features")
                             model.advanced_alpha_predictor[0] = torch.nn.Linear(ckpt_in_features, 128).to(device)
                         
                         model.advanced_alpha_predictor.load_state_dict(checkpoint['advanced_alpha_predictor'])
                    elif 'prompt_shifter' in checkpoint:
                         model.prompt_shifter.load_state_dict(checkpoint['prompt_shifter'])
                    print("Loaded DPAS model.")
                else:
