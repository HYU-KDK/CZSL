...
print(f"DEBUG: CUDA_VISIBLE_DEVICES is {os.environ.get('CUDA_VISIBLE_DEVICES')}")
import pickle
import pprint
import json
...
def train_model(model, optimizer, train_dataloader, config, accelerator, start_epoch=0):
    ...
    #
    attr2idx = train_dataloader.dataset.attr2idx
    obj2idx = train_dataloader.dataset.obj2idx

    train_pairs = torch.tensor([(attr2idx[attr], obj2idx[obj])
                                for attr, obj in train_dataloader.dataset.train_pairs]).to(accelerator.device)
    i = 0
    train_losses = []
    ...
    for i in range(start_epoch, config.epochs):
        ...
        for bid, batch in enumerate(train_dataloader):
            ...
            # dpas.forward() expects raw images and encodes internally
            if config.experiment_name == 'dpas':
                logits = model(batch_img, train_pairs)
            else:
                batch_feat = accelerator.unwrap_model(model).encode_image(batch_img)
...
def train_dpc(model, optimizer, train_dataloader, config, accelerator, start_epoch=0):
    ...
    eye = torch.eye(num_pairs, device=accelerator.device).bool()
    ...
    for i in range(start_epoch, config.epochs):
        ...
        for bid, batch in enumerate(train_dataloader):
            ...
            pos_scores = logits[torch.arange(batch_size, device=accelerator.device), batch_target]
        ...
        if (i + 1) % config.save_every_n == 0:
            if accelerator.is_main_process:
                if config.experiment_name in ['dpc', 'dpc_alpha', 'dpas']:
                    save_dpc_embeddings(model, config, accelerator, epoch=i + 1)
                else:
                    save_soft_embeddings(model, config, accelerator, epoch=i + 1)

    return model, optimizer

def save_dpc_embeddings(model, config, accelerator, epoch=None):
    ...
    with torch.no_grad():
        ...
        # Unwrap model to save correct state
        unwrapped_model = accelerator.unwrap_model(model)
        # Save both primitives, contextuals, and gating param
        state = {
            "soft_embeddings_primitive": unwrapped_model.soft_embeddings_primitive,
            "soft_embeddings_contextual": unwrapped_model.soft_embeddings_contextual,
            "gating_param": unwrapped_model.gating_param
        }
        
        if config.experiment_name == 'dpc_alpha':
            state["alpha_predictor"] = unwrapped_model.alpha_predictor.state_dict()
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        if accelerator.is_main_process:
            ...
            # print(f"DEBUG: GPU Memory Free: {torch.cuda.mem_get_info(0)[0] / 1024**2:.2f} MiB")
    ...
    if config.resume_path is not None:
        if os.path.exists(config.resume_path):
            ...
            if config.experiment_name in ['dpc', 'dpc_alpha', 'dpas']:
                if 'soft_embeddings_primitive' in checkpoint:
                    ...
                    print("Fallback: Loading 'soft_embeddings' into both primitive and contextual.")
                    model.soft_embeddings_primitive.data = checkpoint['soft_embeddings'].data
                    model.soft_embeddings_contextual.data = checkpoint['soft_embeddings'].data
                ...
                elif config.experiment_name == 'dpas' and 'advanced_alpha_predictor' in checkpoint:
                     model.advanced_alpha_predictor.load_state_dict(checkpoint['advanced_alpha_predictor'])
                elif config.experiment_name == 'dpas' and 'prompt_shifter' in checkpoint:
                     model.prompt_shifter.load_state_dict(checkpoint['prompt_shifter'])
            else:
                accelerator.unwrap_model(model).set_soft_embeddings(checkpoint['soft_embeddings'])
    ...
    if accelerator.is_main_process:
        print("model dtype", accelerator.unwrap_model(model).dtype)
        if hasattr(accelerator.unwrap_model(model), 'soft_embeddings'):
            print("soft embedding dtype", accelerator.unwrap_model(model).soft_embeddings.dtype)
        elif hasattr(accelerator.unwrap_model(model), 'soft_embeddings_primitive'):
            print("soft embedding dtype", accelerator.unwrap_model(model).soft_embeddings_primitive.dtype)
    ...
    if config.experiment_name in ['dpc', 'dpc_alpha', 'dpas']:
        save_dpc_embeddings(model, config, accelerator, epoch=config.epochs)
    else:
    ...
    if config.save_model and accelerator.is_main_process:
        torch.save(
            accelerator.unwrap_model(model).dict(),
            os.path.join(
                config.save_path,
                'final_model.pt'))
